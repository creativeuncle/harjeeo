...
// resourceType: "image" | "video" (used for audio too) | "raw" (pdf, csv, other files)
export async function uploadToCloudinary(file, resourceType) {
  if (!isCloudinaryConfigured()) {
    throw new Error(
      "File upload isn't configured yet. Set VITE_CLOUDINARY_CLOUD_NAME and VITE_CLOUDINARY_UPLOAD_PRESET."
    );
  }
